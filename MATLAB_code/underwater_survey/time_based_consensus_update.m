function agent_self = time_based_consensus_update(agent, prev_agents, neighboring_agent_indices, beta)
    % beta = 0.3;
    prev_A = agent.A;
        ...
        for a_l = 1:N_agents
            if a_l ~= agent.id
                if prev_agents(a_k).agent_update_time(a_l) > agent.agent_update_time(a_l)
                    ...
                    agent.completion_time_vec(a_l) = prev_agents(a_k).completion_time_vec(a_l);
                end
            end
        end
    end
