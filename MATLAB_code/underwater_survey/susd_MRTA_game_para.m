function [agents, total_utility, paths_history, utility_history, t] = susd_MRTA_game_para(P, Y, agents, G_comm, N_timesteps, hyperparams)
    gamma = 0.00;
    ...
    for t = 1:N_timesteps
        ...
        parfor a_i = 1:N_agents
            neighboring_agent_indices = neighbors(G_comm, a_i)'; % neighbors
            agents(a_i) = time_based_consensus_update(agents(a_i), prev_agents, neighboring_agent_indices, 0.3);
            % agents(a_i) = leader_following_consensus_update(agents(a_i), prev_agents, neighboring_agent_indices, 0.1);
        end
        ...
        parfor a_i = 1:N_agents
            ...
            if update == 1
                ...
                prev_exec_time = agents(a_i).completion_time_vec(a_i);
                agents(a_i) = action_update_susd(agents(a_i), P, Y, susd_hyperparams, N_susd_steps);
                if any(prev_assigments ~= agents(a_i).assignments) ||  prev_exec_time ~= agents(a_i).completion_time_vec(a_i)
