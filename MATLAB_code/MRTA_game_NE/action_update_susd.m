function agent_self = action_update_susd(agent, P, Y, hyperparams, N_steps)
    ...
    vagents = agent([]);
    for v_i=1:N_vagents
        theta_(:,v_i) = agent.theta'+ rand(N_param,1);
        vagents(v_i) = agent;
    end
    ...
    for t = 1:N_steps
        ...
        for v_i=1:N_vagents
            ...
            assignments = theta_to_assignment(vagents(v_i).theta, vagents(v_i).max_num_tasks, 'L_largest');
...
%         theta_ = theta_./sum(theta_);
        ...
        assignments = theta_to_assignment(vagents(zarg).theta, vagents(zarg).max_num_tasks, 'L_largest');
    ...
    assignments = theta_to_assignment(theta, agent.max_num_tasks, 'L_largest');
