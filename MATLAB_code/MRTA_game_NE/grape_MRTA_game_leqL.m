...
while a_satisfied~=n && time_step < N_timesteps
    ...
    for i=1:n % For Each Agent          
        ...
        % updated_assignment = zeros(1, m); 
        % curr_utility = 0;
        % while sum(updated_assignment) < l
        %     Candidate = ones(m,1)*(-inf);
        %     for t=1:m
        %         if updated_assignment(t) == 0
        %             poss_assignment = updated_assignment;
        %             poss_assignment(t) = 1;
        %             poss_agent = agents(i);
        %             poss_agent.A = Alloc_;
...
%       (Revision) To find out the local "deciding agent" amongst neighbour agents
        ...
        if min(agents(i).A == agents_(i).A, [], 'all')==1 % If local information is changed
        else
